...
import com.Suzj.extraenchantments.Transformer.helper.ASMHelper;
import com.Suzj.extraenchantments.Transformer.util.TransformUtil;
...
public class SMEASM implements IClassTransformer
{
	...
	public byte[] transform(String name, String transformedName, byte[] basicClass)
	{
		...
		if(transformedName.equals("bettercombat.mod.EUtil.Helpers"))
		{
	...
	private byte[] patchEnchantmentHelper(byte[] basicClass)
	{
		...
		for(MethodNode m : clazzNode.methods)
		{
			...
			{
				hookAllIntegerReturn(m, "com/Suzj/extraenchantments/Hook/HookHelper", "modifyFishingLuckBonus", "(ILnet/minecraft/item/ItemStack;)I", aload(0));
			}
			else if(m.name.equals("func_191528_c") || m.name.equals("getFishingSpeedBonus"))
			{
				hookAllIntegerReturn(m, "com/Suzj/extraenchantments/Hook/HookHelper", "modifyFishingSpeedBonus", "(ILnet/minecraft/item/ItemStack;)I", aload(0));
			}
			else if(m.name.equals("func_185283_h") || m.name.equals("getLootingModifier"))
			{
				hookAllIntegerReturn(m, "com/Suzj/extraenchantments/Hook/HookHelper", "modifyLootingModifier", "(ILnet/minecraft/entity/EntityLivingBase;)I", aload(0));
			}
		}
	...
	private byte[] patchEnchantmentProtection(byte[] basicClass)
	{
		...
		for(MethodNode m : clazzNode.methods)
		{
			...
			{
				hookAllDoubleReturn(m, "com/Suzj/extraenchantments/Hook/HookHelper", "modifyBlastDamageReduction", "(DLnet/minecraft/entity/EntityLivingBase;)D", aload(0));
			}
		}
	...
	private void hookArthropod(MethodNode m)
	{
		...
		insert.add(new MethodInsnNode(
			Opcodes.INVOKESTATIC,
                "com/Suzj/extraenchantments/Hook/HookArthropod",
			"hookArthropod",
